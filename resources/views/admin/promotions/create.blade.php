            ...
            <form method="POST" action="{{ route("admin.promotions.store") }}" enctype="multipart/form-data">
                ...
                <div class="form-row">
                    <div class="form-group col-md-4">
                        ...
                        <input class="form-control {{ $errors->has('promo_code') ? 'is-invalid' : '' }}" type="text" name="promo_code" id="promo_code" value="{{ old('promo_code', '') }}" required>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control {{ $errors->has('promo_type') ? 'is-invalid' : '' }}" name="promo_type" id="promo_type">
                            ...
                            @foreach(App\Models\Promotion::PROMO_TYPE_SELECT as $key => $label)
                                <option value="{{ $key }}" {{ old('promo_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group col-md-4">
                        ...
                        <input class="form-control {{ $errors->has('promo_value') ? 'is-invalid' : '' }}" type="number" name="promo_value" id="promo_value" value="{{ old('promo_value', '') }}">
