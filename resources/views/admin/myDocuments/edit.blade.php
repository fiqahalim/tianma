...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.my-documents.update", [$myDocument->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="document_name">{{ trans('cruds.myDocument.fields.document_name') }}</label>
                <input class="form-control {{ $errors->has('document_name') ? 'is-invalid' : '' }}" type="text" name="document_name" id="document_name" value="{{ old('document_name', $myDocument->document_name) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $myDocument->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('agents') ? 'is-invalid' : '' }}" name="agents_id" id="agents_id">
                    @foreach($agents as $id => $entry)
                        <option value="{{ $id }}" {{ (old('agents_id') ? old('agents_id') : $myDocument->agents->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
