...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">
                Edit {{ trans('cruds.masterSetting.fields.location') }}
            </li>
        </ol>
    </nav>
    ...
    <div class="card">
        <div class="card-header font-weight-bold">
            {{ trans('global.edit') }} {{ trans('cruds.masterSetting.fields.location') }}
        </div>
            ...
            <form method="POST" action="{{ route("admin.locations.update", [$location->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <input class="form-control {{ $errors->has('postcode') ? 'is-invalid' : '' }}" type="number" name="postcode" id="postcode" value="{{ old('postcode', $location->postcode) }}" step="1">
                ...
                <div class="form-row">
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control {{ $errors->has('address_1') ? 'is-invalid' : '' }}" type="text" name="address_1" id="address_1" value="{{ old('address_1', $location->address_1) }}">
                    ...
                    {{-- <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="number" name="status" id="status" value="{{ old('status', $location->status) }}" step="1">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('property_names') ? 'is-invalid' : '' }}" name="property_names[]" id="property_names" multiple>
                        @foreach($property_names as $id => $property_name)
                            <option value="{{ $id }}" {{ (in_array($id, old('property_names', [])) || $location->property_names->contains($id)) ? 'selected' : '' }}>{{ $property_name }}</option>
                        @endforeach
                    </select>
