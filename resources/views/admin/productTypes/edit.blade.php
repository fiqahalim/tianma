    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">
                Edit {{ trans('cruds.productType.title_singular') }}
            </li>
        </ol>
    </nav>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.product-types.update", [$productType->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('property_name') ? 'is-invalid' : '' }}" type="text" name="property_name" id="property_name" value="{{ old('property_name', $productType->property_name) }}" required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('building_types') ? 'is-invalid' : '' }}" name="building_types[]" id="building_types" multiple>
                        @foreach($building_types as $id => $building_type)
                            <option value="{{ $id }}" {{ (in_array($id, old('building_types', [])) || $productType->building_types->contains($id)) ? 'selected' : '' }}>{{ $building_type }}</option>
                        @endforeach
                    </select>
