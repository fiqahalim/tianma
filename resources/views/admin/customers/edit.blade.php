...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Edit {{ trans('cruds.customer.title') }}</li>
    </ol>
</nav>
        ...
        <form method="POST" action="{{ route("admin.customers.update", [$customer->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control {{ $errors->has('id_type') ? 'is-invalid' : '' }}" name="id_type" id="id_type">
                        ...
                        @foreach(App\Models\Customer::ID_TYPE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('id_type', $customer->id_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-row">
                <div class="form-group col-md-2">
                    ...
                    <input class="form-control {{ $errors->has('postcode') ? 'is-invalid' : '' }}" type="number" name="postcode" id="postcode" value="{{ old('postcode', $customer->postcode) }}" step="1">
