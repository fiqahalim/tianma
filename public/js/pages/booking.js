...
function selectSeat() {
  ...
  var seatDetails = ""; // let price = $('input[name=price]').val();
  // let subtotal = 0;
  ...
  if (selectedSeats.length > 0) {
    ...
    $.each(selectedSeats, function (i, value) {
      seats += $(value).data('seat') + ','; // seatDetails += `<span class="list-group-item d-flex justify-content-between">${$(value).data('seat')} <span>${price} ${currency}</span></span>`;
      // subtotal = subtotal + parseFloat(price);
    });
    $('input[name=seat]').val(seats); // $('.selected-seat-details').html(seatDetails);
